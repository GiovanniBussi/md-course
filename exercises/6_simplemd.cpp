           ...
           int&    idum)
{
  ...
  while(fgets(buffer,256,fp)){
    ...
    else if(keyword=="lr")
    {
      ...
      if(buffer1[0]=='T' || buffer1[0]=='t') lr=true;
    }
...
void compute_forces(const int natoms,const int listsize,const vector<Vector>& positions,const double cell[3],
                    double forcecutoff,const vector<int>& point,const vector<int>& list,vector<Vector>& forces,double & engconf,bool lr)
{
  Vector distance;        // distance of the two atoms
  Vector distance_pbc;    // minimum-image distance of the two atoms
...
// same force on the two atoms, with opposite sign:
  ...
  if(lr) engconf+=-0.5*natoms*(natoms-1)*4*3.14159265359/(3*std::pow(forcecutoff,3)*cell[0]*cell[1]*cell[2]);
}
...
void write_statistics(const string & statfile,const int istep,const double tstep,
                      const int natoms,const double engkin,const double engconf,const double engint,double cell[3]){
...
// every 100 steps, reopen the file to flush the buffer
  ...
  fprintf(write_statistics_fp,"%d %f %f %f %f %f %f\n",istep,istep*tstep,2.0*engkin/(3.0*natoms),engconf,engkin+engconf,engkin+engconf+engint,cell[0]*cell[1]*cell[2]);
}
...
// all of them have a reasonable default value, set in read_input()
  ...
  read_input(stdin,temperature,tstep,friction,lr,pressure,forcecutoff,
...
// forces are computed before starting md
  compute_forces(natoms,listsize,positions,cell,forcecutoff,point,list,forces,engconf,lr);
...
// kinetic energy is calculated
  ...
  const double deltaV=10.0;

  const double vol=cell[0]*cell[1]*cell[2];
  const double newvol=vol+deltaV*(2*random.U01()-1.0);

  const double scale_factor=std::pow(newvol/vol,1.0/3.0);
  double newcell[3];

  std::vector<Vector> newpos(positions);
  std::vector<Vector> newfor(forces);
  double neweng;

  for(unsigned i=0;i<3;i++) newcell[i]=cell[i]*scale_factor;

  for(int iatom=0;iatom<natoms;iatom++) for(int k=0;k<3;k++) {
     newpos[iatom][k]*=scale_factor;
  }

  compute_forces(natoms,listsize,newpos,newcell,forcecutoff,point,list,newfor,neweng,lr);

  const double deltae=(neweng-engconf+pressure*(newvol-vol))/temperature-natoms*std::log(newvol/vol);
  const double acceptance=(deltae<0.0? 1.0 : std::exp(-deltae));

  if(acceptance>random.U01()) {
    positions=newpos;
    forces=newfor;
    engconf=neweng;
    cell[0]=newcell[0];
...
// eventually, write positions and statistics
    ...
    if((istep+1)%nstat==0)   write_statistics(statfile,istep+1,tstep,natoms,engkin,engconf,engint,cell);
