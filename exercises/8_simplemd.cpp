           ...
           int&    idum)
{
  ...
  while(fgets(buffer,256,fp)){
    ...
    if(keyword=="temperature")
      ...
      sscanf(line.c_str(),"%s %lf",buffer,&lambda);
    else if(keyword=="eta")
      sscanf(line.c_str(),"%s %lf",buffer,&eta);
    else if(keyword=="tau")
      sscanf(line.c_str(),"%s %lf",buffer,&tau);
...
void compute_forces(const int natoms,const int listsize,const vector<Vector>& positions,const double cell[3],
                    double forcecutoff,const vector<int>& point,const vector<int>& list,vector<Vector>& forces,double & engconf, double & Rgyr,double & lambda, double eta, double tau, double temperature,double time)
{
  Vector distance;        // distance of the two atoms
  Vector distance_pbc;    // minimum-image distance of the two atoms
...
// same force on the two atoms, with opposite sign:
  ...
  for(int i=0;i<natoms-1;i++) {
      Vector distance,distance_pbc;
      Vector f;
      double distance_pbc2;
      for(int k=0;k<3;k++) distance[k]=positions[i+1][k]-positions[i][k];
      pbc(cell,distance,distance_pbc);
      distance_pbc2=0.0; for(int k=0;k<3;k++) distance_pbc2+=distance_pbc[k]*distance_pbc[k];
      engconf+=-0.5*Kappa*R0*R0*std::log(1.-distance_pbc2/(R0*R0));
      for(int k=0;k<3;k++) f[k]=-Kappa/(1.-distance_pbc2/(R0*R0)) * distance[k];
      for(int k=0;k<3;k++) forces[i+1][k]+=f[k];
      for(int k=0;k<3;k++) forces[i][k]-=f[k];
  }

  std::vector<Vector> positions_whole(natoms);
  positions_whole[0]=positions[0];
  for(int i=0;i<natoms-1;i++) {
    Vector distance=positions[i+1]-positions[i];
    pbc(cell,distance,distance_pbc);
    positions_whole[i+1] = positions_whole[i] + distance_pbc;
  }
  
  Vector center(0.0,0.0,0.0);
  for(int i=0;i<natoms;i++) {
    center+=positions_whole[i];
  }
  center*=(1.0/natoms);

  double Rgyr2=0.0;
  for(int i=0;i<natoms;i++) {
    Rgyr2+=modulo2(positions_whole[i]-center);
  }
  Rgyr=std::sqrt(Rgyr2/natoms);

  lambda += eta/(time+tau)*(Rgyr-2.5);

  engconf += lambda * temperature * Rgyr;

  for(int i=0;i<natoms;i++) {
    Vector f=-(positions_whole[i]-center)/natoms/Rgyr * lambda * temperature;
    forces[i]+=f;
  }
...
void write_statistics(const string & statfile,const int istep,const double tstep,
                      const int natoms,const double engkin,const double engconf,const double engint,const double Rgyr,double lambda){
...
// every 100 steps, reopen the file to flush the buffer
  ...
  fprintf(write_statistics_fp,"%d %f %f %f %f %f %f %f\n",istep,istep*tstep,2.0*engkin/(3.0*natoms),engconf,engkin+engconf,engkin+engconf+engint,Rgyr,lambda);
}
...
// all of them have a reasonable default value, set in read_input()
  ...
  read_input(stdin,temperature,lambda,eta,tau,tstep,friction,forcecutoff,
...
// forces are computed before starting md
  compute_forces(natoms,listsize,positions,cell,forcecutoff,point,list,forces,engconf,Rgyr,lambda,eta,tau,temperature,0.0);
...
// a check is performed to decide whether to recalculate the neighbour list
    ...
    compute_forces(natoms,listsize,positions,cell,forcecutoff,point,list,forces,engconf,Rgyr,lambda,eta,tau,temperature,tstep*istep);
...
// eventually, write positions and statistics
    ...
    if((istep+1)%nstat==0)   write_statistics(statfile,istep+1,tstep,natoms,engkin,engconf,engint,Rgyr,lambda);
