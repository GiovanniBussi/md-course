...
// with proper coefficients
   ...
   double engkin=0.0;
   for(int iatom=0;iatom<natoms;iatom++) for(int i=0;i<3;i++){
      engkin+=0.5*masses[iatom]*velocities[iatom][i]*velocities[iatom][i];
   }

   double alpha=std::sqrt(1.5*natoms*temperature/engkin);

   for(int iatom=0;iatom<natoms;iatom++) for(int i=0;i<3;i++){
      velocities[iatom][i]*=alpha;
   }
...
// a check is performed to decide whether to recalculate the neighbour list
    ...
    if((istep+1)%20==0) thermostat(natoms,masses,0.5*tstep,friction,temperature,velocities,engint,random);
